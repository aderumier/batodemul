...
// With small changes for R-Type Dimensions                                 //
//**************************************************************************//
...
uniform float3 test1_color1 <
	ui_type = "color";
	ui_label = "1st Pixel Color 1 (RGB)";
	ui_category = "Pixel Test Advanced Settings";
> = float3(0.0, 0.0, 0.0);

uniform float3 test2_color1 <
	ui_type = "color";
	ui_label = "2nd Pixel Color 1 (RGB)";
	ui_category = "Pixel Test Advanced Settings";
> = float3(0.0, 0.0, 0.0);

#ifdef DBLE_TESTS
uniform float3 test1_color2 <
	ui_type = "color";
	ui_label = "1st Pixel Color 2 (RGB)";
	ui_category = "Pixel Test Advanced Settings";
	ui_spacing = 1;
> = float3(0.0, 0.0, 0.0);

uniform float3 test2_color2 <
	ui_type = "color";
	ui_label = "2nd Pixel Color 2 (RGB)";
	ui_category = "Pixel Test Advanced Settings";
> = float3(0.0, 0.0, 0.0);
#endif
...
texture bezel2_texture <source="bezel2.png";> { Width = 1920; Height = 1080; Format = RGBA8; };
sampler bezel2_sampler { Texture = bezel2_texture; AddressU = BORDER; AddressV = BORDER; AddressW = BORDER; };
...
// Pixel Tests
int PIXELTESTS(float2 video_size, float2 full_video_size)
{
	...
	if (PIXEL_TEST)
	{
		...
		float3 delta = tex2D(ReShade::BackBuffer, (test_pixel1 * scale / Standard_size + offset)).rgb;
		float3 delta2 = tex2D(ReShade::BackBuffer, (test_pixel2 * scale / Standard_size + offset)).rgb;
		if ((test_epsilon < dot(delta - test1_color1, delta - test1_color1)) || (test_epsilon < dot(delta2 - test2_color1, delta2 - test2_color1)))
			TEST = TEST - 1;
#ifdef DBLE_TESTS
		int TEST2 = 2;
		if ((test_epsilon < dot(delta - test1_color2, delta - test1_color2)) || (test_epsilon < dot(delta2 - test2_color2, delta2 - test2_color2)))
			TEST2 = TEST2 - 2;
		TEST = TEST + TEST2;
#endif
	}
...
{
	float2 full_video_size = GetSize(GameVideo_resolution, FullScreen_size);
	int PIXEL_TESTS = PIXELTESTS(VideoSize(full_video_size), full_video_size);
	float2 CropFix = CropRatioFix(PIXEL_TESTS, full_video_size);
	float4 starts = Starts(full_video_size, CropFix);
	...
	if (USE_BEZEL)
	{
		...
		bezel = tex2D(bezel_off_sampler, bezel_uv);
		float4 bezelx = 0.0f;
		if (PIXEL_TESTS == 1)
			bezelx = tex2D(bezel_sampler, bezel_uv);
		else if (PIXEL_TESTS == 2)
			bezelx = tex2D(bezel2_sampler, bezel_uv);
		bezel.rgb = lerp(bezel.rgb, bezelx.rgb, bezelx.a);
	}
#endif
	// True Game UV
